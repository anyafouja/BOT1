...
async def refresh_cookies() -> bool:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get('https://www.youtube.com/', headers=headers) as resp:
                cookies = session.cookie_jar.filter_cookies('https://www.youtube.com')
                if not cookies:
                    return False
                now = int(time.time())
                ...
                for name, cookie in cookies.items():
                    ...
                    exp = int(cookie.get('expires', expiry))
                    lines.append(f'{domain}\t{flag}\t{path}\t{secure}\t{exp}\t{name}\t{cookie.value}')
